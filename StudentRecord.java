...
class StudentRecord
{
	...
	public static void main(String[] args)
	{
		...
		StudentRecord clr = new StudentRecord();

		/*Association of other classes*/

		Student[] s = new Student[30]; 
		StudentPrevRecords sh = new StudentPrevRecords();
		Placements p = new Placements();
		Courses cor = new Courses();
		Results re = new Results();
		Student num = new Student();
		StudentConduct cndt = new StudentConduct();

		/* Event Objects */


		ExtraEvents cul = new ExtraEvents();
		OtherEvents othr = new OtherEvents();
		TechnicalEvents tf  = new TechnicalEvents();

		ExtraEvents cul_temp = new ExtraEvents();
		OtherEvents othr_temp = new OtherEvents();
		TechnicalEvents tf_temp = new TechnicalEvents();
		...
		System.out.println("<<<<<<<<<<<<<<<---Main Menu--->>>>>>>>>>>>>\n\n1----> Add_Student(s)\n2----> Student Diary Of Student(Search)\n3---->To Display All Students");
		...
		if(option == 1)
		{
				...
				for(int i=1;i<=n;++i)   //Begining of the for loop
				{
					...
					System.out.print("Do u want to add a misactivity record ? (1 for yes) : ");
					int o = sc.nextInt();
					
						if(o==1)
						{
							cndt.Suspensioncause(); //Setting Suspension Record (o(1))
						}
			    ...
			    do{
					...
					try
					{
						...
						/* Reading Objects */
		      	 		...
		      	 		sh    = (StudentPrevRecords) oos.readObject();
		      	 		...
		      	 		cndt  = (StudentConduct) oos.readObject();
								...
								System.out.flush();
				      	 		...
				      	 		else if(opt==3) //EventsParticipated
				      	 		{
										...
										System.out.flush();
								      	...
								      	if(num.flag==1)
										{
											tf_temp  = (TechnicalEvents) oos.readObject();
											tf_temp.Get_Details();
										}
										else if(num.flag==2)
										{		
											cul_temp   = (ExtraEvents) oos.readObject();
